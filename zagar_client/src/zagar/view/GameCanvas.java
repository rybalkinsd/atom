...
public class GameCanvas extends JPanel {
  ...
  public static final Logger log = LogManager.getLogger(GameCanvas.class);
  ...
  protected void paintComponent(Graphics g) {
    log.debug("paintComponentCall();");
    super.paintComponent(g);
    render();
  }

  public void render() {
    log.debug("CANVAS RENDER");
    ...
    if (Game.player.size() > 0 && Game.zoom > 0.0) {
      ...
      log.debug("RENDER GRID");
      log.debug("ZOOM: " + Game.zoom);
      log.debug("AVGX: " + avgX + "; AVGY: " + avgY);
      int j;
      for (
              double i = avgX - (GameFrame.size.width / 2) / Game.zoom;
              i < avgX + (GameFrame.size.width / 2) / Game.zoom;
              i += 100
              ) {
        j = (int) (i / 100) * 100;
//        i = (int) (i / 100) * 100;
        int x = (int) ((j - avgX) * Game.zoom) + GameFrame.size.width / 2 - size / 2;
        g.drawLine((int) x, (int) Game.minSizeY, (int) x, (int) Game.maxSizeY);
      }
      for (
              double i = avgY - (GameFrame.size.height / 2) / Game.zoom;
              i < avgY + (GameFrame.size.height / 2) / Game.zoom;
              i += 100
              ) {
        j = (int) (i / 100) * 100;
        int y = (int) ((j - avgY) * Game.zoom) + GameFrame.size.height / 2 - size / 2;
