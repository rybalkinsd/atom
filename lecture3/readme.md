...
1. Сервер
	...
	- расположенный на определенном сетевом адресе.
	- умеет принимать запросы:
...
2. Клиент:
	- код клиента назодится в ru.atom.client
	- расположен на локальной машине студента
	- хочет делать запросы через сеть на сервер:
		- хочет зарегистрироваться
		- хочет получить набор данных
	- реализация некоторых запросов *отсутствует*

##### Задача
Реализовать недостающие методы клиента, которые бы осуществляли необходимое общение с сервером.

##### Зачем?
В реальных приложения очень часто встречается использование публичных API.
Например: 
...
Будем считать, что сервер, в нашем случае, это некоторый публичный API.
Сервер имеет некоторый адрес в сети - IP(127.0.0.1 или localhost при локальном тестировании).

Клиент и сервер должны как-то общаться.
Через сеть.
Для межсетевого взаимодействия мы выбрали протокол HTTP.
HTTP - это сетевой протокол прикладного уровня (его используют приложения на компьютерах).
...
Наш сервер предоставляет публичный API:
...
Сервер находится в пакете ru.atom.server - Server.
В нем есть метод main, который стартует сервер.

Тесты клиента лежат в пакете ru.atom.client - RestClientImplTest
