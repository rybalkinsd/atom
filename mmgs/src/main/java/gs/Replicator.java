...
public class Replicator {
    ...
    public void writeReplica(GameSession gs, ConnectionPool connectionPool){
        ...
        if (gs.jsonStringBombs() == null) {
            ...
            Broker.getInstance().send(gs, connectionPool.getPlayer(session), Topic.REPLICA, gs.jsonStringWalls() + "," + gs.jsonStringBombs() + "," + gs.getPawn().toJson());
        } else {
            Broker.getInstance().send(gs, connectionPool.getPlayer(session), Topic.REPLICA, gs.jsonStringWalls() + "," + gs.jsonStringBombs() + "," + gs.jsonStringExplosions() + "," + gs.getPawn().toJson());
        }
    }
}
