...
public class ChatApplication {
    ...
    public Queue<Triplet<String, Date, String>> loadHistory() {
        ...
        String date = "";
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(ChatApplication.class
                .getClassLoader().getResource("history.txt").getPath()))
        ) {
            String string;
            while ((string = bufferedReader.readLine()) != null) {
                SimpleDateFormat simpleDateFormat = new SimpleDateFormat("HH:mm:ss");
                date = string.split(" ")[0];
                String[] other = string.substring(9).split(":");
                String msg = "";
